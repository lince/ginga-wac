...
namespace core {
namespace player {
	AVPlayer::AVPlayer(string mrl, bool hasVisual) : Thread(), ESPlayer(mrl) {
		this->provider = NULL;
		this->mainAV = false;
		...
		if (mrl.substr(0, 11) == "sbtvd-ts://") {
			this->mainAV = true;
			this->mrl = mrl.substr(11, mrl.length());
			Thread::start();

		} else if (fileExists(mrl)) {
			if (hasVisual) {
				this->provider = new VideoProvider((char*)(mrl.c_str()));
			} else {
				this->provider = new AudioProvider((char*)(mrl.c_str()));
			}
	...
	void AVPlayer::setVoutWindow(Window* win) {
		if (mainAV) {
			this->win = win;
		}
	}
	...
	void AVPlayer::run() {
		lock();
		if (mainAV) {
			this->provider = new VideoProvider((char*)(this->mrl.c_str()));
			running = true;
			unlock();
			this->surface = createFrame();
			lock();
			...
			::usleep(1000000);
			while (dur > getMediaTime() && getMediaTime() > 0) {
				if (!this->usleep((long)((dur - getMediaTime()) * 1000))) {
					break;
				}
			}
		}
