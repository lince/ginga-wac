...
namespace player {
	Player::Player(string mrl) {
		...
		pthread_mutex_init(&addListM, NULL);
		pthread_mutex_init(&remListM, NULL);
		pthread_mutex_init(&referM, NULL);

		this->mrl = mrl;
		this->listeners = NULL;
		this->toAddListeners = NULL;
		this->toRemListeners = NULL;
	...
	Player::~Player() {
		...
		//pthread_mutex_lock(&addListM);
		if (toAddListeners != NULL) {
			toAddListeners->clear();
			delete toAddListeners;
			toAddListeners = NULL;
		}

		//pthread_mutex_lock(&remListM);
		if (toRemListeners != NULL) {
			toRemListeners->clear();
			delete toRemListeners;
			toRemListeners = NULL;
		}
		...
		pthread_mutex_unlock(&addListM);
		pthread_mutex_unlock(&remListM);
		pthread_mutex_unlock(&listM);

		pthread_mutex_destroy(&referM);
		pthread_mutex_destroy(&addListM);
		pthread_mutex_destroy(&remListM);
	...
	void Player::addListener(IPlayerListener* listener) {
		if (notifying) {
			pthread_mutex_lock(&addListM);
			if (toAddListeners == NULL) {
				toAddListeners = new set<IPlayerListener*>;
			}
			toAddListeners->insert(listener);
			pthread_mutex_unlock(&addListM);
	...
	void Player::removeListener(IPlayerListener* listener) {
		...
		if (notifying) {
			pthread_mutex_lock(&remListM);
			if (toRemListeners == NULL) {
				toRemListeners = new set<IPlayerListener*>;
			}
			toRemListeners->insert(listener);
			pthread_mutex_unlock(&remListM);
	...
	void Player::notifyListeners(short code, string parameter, short type) {
		...
		pthread_mutex_lock(&remListM);
		if (toRemListeners != NULL && listeners != NULL) {
			i = toRemListeners->begin();
			while (i != toRemListeners->end()) {
				j = listeners->find(*i);
				if (j != listeners->end()) {
					listeners->erase(j);
				}
		...
		if (toAddListeners != NULL) {
			if (listeners == NULL) {
				listeners = new set<IPlayerListener*>;
			}

			i = toAddListeners->begin();
			while (i != toAddListeners->end()) {
				listeners->insert(*i);
				++i;
			}
			toAddListeners->clear();
	...
	bool Player::setFocusHandler(bool isHandler) {
		if (isHandler) {
			::usleep(200000);
		}
		return false;
	...
	PlayerState* Player::getPlayerState() {
		...
		ps->setMediaTime(this->getMediaTime());
		ps->setVisible(visible);
		ps->setPresented(presented);
		int w, h;
		ps->setWidth(w);
		ps->setHight(h);
		ps->setStatus(status);
