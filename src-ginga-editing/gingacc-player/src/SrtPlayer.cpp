...
namespace core {
namespace player {
	SrtPlayer::SrtPlayer() : TextPlayer(), Thread() {
		this->mrl = "";
...
//				wclog << " text = '" << text << "'" << endl;
	...
	void SrtPlayer::setReferenceTimePlayer(Player* player) {
		lock();
		this->player = (AVPlayer*)player;
		unlock();
	}
	...
	void SrtPlayer::run() {
		...
		if (surface != NULL) {
			...
			if (parent != NULL) {
				...
				wclog << "SrtPlayer::run warning! parent == NULL" << endl;
			}
...
//				wclog << "' sleepTime = '" << sleepTime << "'" << endl;
				...
				if (surface != NULL) {
					...
					if (parent != NULL && controlParentVisibility) {
						wclog << "SrtPlayer::run ";
						wclog << "HIDE LINE: '" << line.c_str() << "'" << endl;
						parent->hide();
					}
				}
				unlock();
