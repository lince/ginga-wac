...
namespace player {
	AnimePlayer::AnimePlayer(vector<string>* mrls) : Thread::Thread() {
		...
		running = false;
		win = NULL;
		anime = new vector<io::IImageProvider*>;
		if (cm != NULL) {
			i = mrls->begin();
			while (i != mrls->end()) {
				mrl = *i;
	...
	AnimePlayer::~AnimePlayer() {
		vector<io::IImageProvider*>::iterator i;
	...
	void AnimePlayer::setVoutWindow(io::IWindow* win) {
		this->win = win;
	}
	...
	io::ISurface* AnimePlayer::prepareSurface(
			io::IImageProvider* provider) {

		io::ISurface* renderedSurface = NULL;
