...
set<IDirectFBVideoProvider*>* DFBVideoProvider::pool = (
	...
	ISurface* DFBVideoProvider::getPerfectSurface() {
		...
		return new DFBSurface(
				DeviceManager::getInstance()->createSurface(&dsc));
	}

	void DFBVideoProvider::dynamicRenderCallBack(void* rendererContainer) {
		IDirectFBVideoProvider* dec = NULL;
		IDirectFBWindow* w = NULL;
		IDirectFBSurface* s = NULL;
		IDirectFBSurface* frame = NULL;
		DFBVideoProviderStatus st;

		RendererContainer* cont = NULL;
		...
		cont = (RendererContainer*)rendererContainer;
