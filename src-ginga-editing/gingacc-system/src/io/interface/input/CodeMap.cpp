...
namespace io {
	...
	//const int CodeMap::KEY_MODE_CHANGE = 61458;
	//const int CodeMap::KEY_MODE_CHANGE = 61706;
...
/*
		...
		(*valueMap)[KEY_SMALL_X] = "x";if (!hasCode(code)) {
			return false;
		}
		string corStr = getValue(code);
		if (keyMap->erase(code) != 0 && keyMap->erase(codeStr) != 0) {
			return true;
		}
		return false;
