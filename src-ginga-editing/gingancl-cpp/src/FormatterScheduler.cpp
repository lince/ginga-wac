...
namespace ncl {
	...
	void FormatterScheduler::runAction(
		...
		executionObject = (ExecutionObject*)(event->getExecutionObject());
		string id = executionObject->getId();
		/*if (id.find("client") != -1) {
			cerr<<"FormatterScheduler::runAction()"<<endl;
			cerr<<"Evento do cliente: "<<id<<endl;
			//return;
		}*/
		...
		if (dataObject->instanceOf("ContentNode") &&
			...
			if (attValue != "" && attValue.substr(0, 1) == "$") {
			...
			if (attName == "service.currentFocus") {
				focusManager->setFocus(attValue);

			} else if (attName == "service.currentKeyMaster") {
		...
		switch (actionType) {
			...
			case SimpleAction::ACT_SET:
				...
				if (player != NULL && player->hasPrepared()) {
					cout << "FormatterScheduler::runActionOverProperty ";
					cout << "'" << event->getId() << "' call setProp" << endl;
	...
	void FormatterScheduler::initializeDocumentSettings(Node* node) {
		...
		if (node->instanceOf("ContentNode")) {
			...
			if (nodeType == ContentNode::SETTING_NODE) {
	...
	void FormatterScheduler::stopObject(IObjectMode* object) {

	}

	void FormatterScheduler::chanceProperty(IObjectMode* object, 
