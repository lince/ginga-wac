...
#include "../../../../include/adapters/procedural/lua/LuaPlayerAdapter.h"
...
namespace lua {
	LuaPlayerAdapter::LuaPlayerAdapter() : ProceduralPlayerAdapter()  {
	...
	void LuaPlayerAdapter::createPlayer() {
		if (fileExists(mrl)) {
			if (cm != NULL) {
				player = ((PlayerCreator*)(cm->getObject("LuaPlayer")))(
						mrl.c_str(), true);
			}
	...
	void LuaPlayerAdapter::setCurrentEvent(FormatterEvent* event) {
		...
		if (preparedEvents->count(event->getId()) != 0 &&
			...
			currentEvent = event;
			((ProceduralExecutionObject*)object)->setCurrentEvent(currentEvent);
...
extern "C" ::br::pucrio::telemidia::ginga::ncl::adapters::IPlayerAdapter*
		createLuaAdapter(void* param) {

	return new ::br::pucrio::telemidia::ginga::ncl::adapters::procedural::lua::
			LuaPlayerAdapter();
}
