...
namespace emconverter {
	...
	FormatterCausalLink* FormatterLinkConverter::createCausalLink(
		...
		formatterAction = createAction(
			    actionExpression, ncmLink, parentObject, depthLevel, editingType);
		//END CAIO
		...
		//BEGIN CAIO
			formatterAction->setLinkId(ncmLink->getId());
		//END CAIO
	...
	LinkAction* FormatterLinkConverter::createAction(
		...
		if (actionExpression->instanceOf("SimpleAction")) {
			...
			linkAction = createCompoundAction(
				    cae->getOperator(),
				    delay,
				    cae->getActions(),
				    ncmLink,
		...
		linkAction->setLinkType(editingType);
		return linkAction;
		//modificações caio
	}
	...
	string FormatterLinkConverter::getBindKey(Link* ncmLink, Bind* ncmBind) {
		...
		if (keyValue == "") {
			...
			if (auxParam != NULL) {
				...
				cout<<"FormatterLinkConverter::key: "<<key<<endl;
