...
#include "../../../../include/ProgramAVPlayerAdapter.h"
...
namespace tv {
	...
	void ProgramAVPlayerAdapter::createPlayer() {
		...
		int pid;
		string sPid;
		sPid = mrl.substr(mrl.find_first_of("/") + 1, mrl.length());
		pid = (int)(stof(sPid));
		player = ProgramAV::getInstance()->getPlayer(pid);
	...
	bool ProgramAVPlayerAdapter::setPropertyValue(
		...
		if (propName == "size" || propName == "location" ||
			...
			if (object != NULL) {
				...
				ProgramAV::getInstance()->setBounds(
