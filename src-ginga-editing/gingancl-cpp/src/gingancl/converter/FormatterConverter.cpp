...
namespace converter {
	...
	FormatterConverter::~FormatterConverter() {
		wclog << "FormatterConverter::~FormatterConverter" << endl;
		ruleAdapter = NULL;
		scheduler = NULL;
	...
	CompositeExecutionObject* FormatterConverter::addSameInstance(
		...
		try {
			...
			wclog << "FormatterConverter::addSameInstance catched ";
			...
			if (parentObject->instanceOf("ExecutionObjectSwitch")) {
				...
				if (parentSwitch->getSelectedObject() == NULL) {
					...
					if (selectedNode != perspective->getAnchorNode()) {
						wclog << "FormatterConverter::getExecutionObject throw";
						wclog << " new ObjectCreationForbiddenException() ";
						wclog << "because selectedNode != perspective->";
						wclog << "getAnchorNode()" << endl;
					...
					wclog << "FormatterConverter::getExecutionObject throwing ";
					wclog << "new ObjectCreationForbiddenException() ";
					wclog << "parentSwitch->getSelectedObject()->getDataObjec";
					wclog << "t != perspective->getAnchorNode()" << endl;
	...
	CompositeExecutionObject* FormatterConverter::getParentExecutionObject(
		...
		if (perspective->getNumNodes() > 1) {
			...
			wclog << "FormatterConverter::getParentExecutionObject parent";
			wclog << "Perspective id = '";
			wclog << parentPerspective->getId().c_str() << "'" << endl;
			return (CompositeExecutionObject*)(this->getExecutionObject(
	...
	void FormatterConverter::createMultichannelObject(
		...
		if (nodes != NULL) {
			...
			while (i != nodes->end()) {
				...
				wclog << "FormatterConverter::createMultichannelObject id = '";
				wclog << id.c_str() << "'" << endl; 
	...
	ExecutionObject* FormatterConverter::createExecutionObject(
		...
		/*cout << "FormatterConverter::createExecutionObject '" << id << "'";
		cout << " perpective '" << perspective->getId() << "'" << endl;*/
		...
		if (nodeEntity->instanceOf("ContentNode") &&
			    ...
			    !(upperCase(((ContentNode*)nodeEntity)->
			    	    getNodeType()) == upperCase(
					    	    ContentNode::SETTING_NODE))) {
		...
		if (nodeEntity->instanceOf("SwitchNode")) {
			...
			wclog << "FormatterConverter::createExecutionObject creating ";
			wclog << "a new CompositeExecutionObject '" << id.c_str();
			wclog << "'" << endl;
	...
	void FormatterConverter::compileExecutionObjectLinks(
		...
		wclog << "FormatterConverter::compileExecutionObjectLinks compiling ";
		executionObject->setCompiled(true);
		if (parentObject == NULL) {
			return;
		}

		vector<Link*>* dataLinks;
		vector<Link*>* uncompiledLinks;
		unsigned int i;
		Link* link;
		...
		wclog << "uncompiledLinks->size = '";
		if (uncompiledLinks != NULL) {
			wclog << uncompiledLinks->size();
			dataLinks = new vector<Link*>(*uncompiledLinks);
			wclog << "' dataLinks->size = '" << dataLinks->size();
			wclog << "' links: '";

			for (i = 0; i < dataLinks->size(); i++) {
				link = (*dataLinks)[i];
				wclog << link->getId().c_str() << "', '";
				...
				if (parentObject->containsUncompiledLink(link)) {
					...
					if (link->instanceOf("CausalLink")) {
						...
						// link conditions
						if (((CausalLink*)link)->containsSourceNode(
	...
	void FormatterConverter::setActionListener(LinkAction* action) {
		if (action->instanceOf("LinkSimpleAction")) {
			((LinkSimpleAction*)action)->addActionListener(
					...
					if (mappings != NULL) {
						...
						while (j != mappings->end()) {
							...
							if (mapping->getNode() == selectedNode) {
								...
								nodePerspective->append(
									    mapping->getMapNodeNesting());
								...
								try {
									...
									if (endPointObject != NULL) {
									    ...
									    exc) {
									// continue
									wclog << "FormatterConverter::";
									wclog << "resolveSwitchEvents catched ";
									wclog << "ObjectCreationForbidden";
									wclog << "Exception" << endl;
								}
