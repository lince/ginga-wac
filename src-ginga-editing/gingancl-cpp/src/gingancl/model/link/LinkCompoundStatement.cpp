...
#include "../../../include/LinkCompoundStatement.h"
...
namespace link {
	...
	LinkCompoundStatement::~LinkCompoundStatement() {
		wclog << "LinkCompoundStatement::~LinkCompoundStatement" << endl;
	...
	bool LinkCompoundStatement::returnEvaluationResult(bool result) {
		wclog << "LinkCompoundStatement::returnEvaluationResult";
		wclog << " negated = '" << negated << "'" << " result = '";
		wclog << result << "'" << endl;
	...
	bool LinkCompoundStatement::evaluate() {
		...
		if (op == CompoundStatement::OP_OR) {
			wclog << "LinkCompoundStatement::evaluate OP_OR" << endl;
			for (i = 0; i < size; i++) {
			...
			wclog << "LinkCompoundStatement::evaluate OP_AND" << endl;
			for (i = 0; i < size; i++) {
				childStatement = (LinkStatement*)(*statements)[i];
				if (!childStatement->evaluate())
					return returnEvaluationResult(false);
