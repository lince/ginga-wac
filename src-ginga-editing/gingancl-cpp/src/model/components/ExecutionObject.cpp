...
namespace components {
	...
	void ExecutionObject::updateTransitionTable(
			double currentTime, IPlayer* player) {
		...
		if (mainEvent->instanceOf("PresentationEvent")) {
			...
			while (i != transitionTable->end()) {
				...
				if (transition->getTime() > endTime) {
					...
					/*cout << "ExecutionObject::stop for '" << id << "'";
					cout << " call fev->stop(" << fev->getId() << ")" << endl;*/
					fev->stop();
				}
				++i;
			}
		...
		if (anim == NULL || stof(anim->getDuration()) <= 0.0) {
			...
			if (propName == "size") {
				...
				if (params->size() == 2) {
					...
					data1->value.push_back((*params)[0]);
					data1->value.push_back((*params)[1]);
					data1->region = region;
					data1->duration = dur;
					data1->stepSize = stepSize;
					data1->propName.push_back("width");
					data1->propName.push_back("height");
				...
				if (params->size() == 2) {
					...
					data1->duration = dur;
					data1->stepSize = stepSize;
					data1->propName.push_back("left");
					data1->propName.push_back("top");
				...
				if (params->size() == 4) {
					...
					data1 = new struct animeInfo;
					for(int i = 0; i < 4; i++)
						data1->value.push_back((*params)[i]);
					data1->region = region;
					data1->duration = dur;
					data1->stepSize = stepSize;
					data1->propName.push_back("left");
...
//					pthread_join(id4, NULL);
				...
				data1 = new struct animeInfo;
				data1->value.push_back(value);
				data1->region = region;
				data1->duration = dur;
				data1->stepSize = stepSize;
				data1->propName.push_back(propName);
...
//				initTime = getCurrentTimeMillis();
//
//				//get the target of the animation (rg)
//				int target = getAnimationTarget(value, ncmRegion, propName);
//
//				//get the current value of region parameter
//				int initValue = getCurrentValue(ncmRegion, propName);
//
//				while (!done) {
//					done = animeStep(
//							initValue,
...
//				return true;
	...
	void* ExecutionObject::animeThread(void* ptr) {
		...
		for(int i = 0; i < propName.size(); i++)
			target.push_back(getAnimationTarget(value[i], region, propName[i]));

		//get the current value of region parameter (rg)
		for(int i = 0; i < propName.size(); i++)
			initValue.push_back(getCurrentValue(region, propName[i]));


		while (!done) {
			doneTmp = true; //is true if and only if all animeStep call is true (rg)
			for(int i = 0; i < propName.size(); i++){
				//cout<<"ExecutionObject::animeThread propName: " <<propName[i] <<" target:"<< target[i]<< " initValue: "<< initValue[i]<< " stepSize: "<< stepSize <<endl;
				doneTmp = animeStep(initValue[i], target[i], region, initTime, dur, propName[i], stepSize) && doneTmp;
			}
	...
	//added rg
	int ExecutionObject::getAnimationTarget(
			string value,
			LayoutRegion *region,
			string param
			){
		...
		//cout<<"ExecutionObject::getAnimationTarget  value="<<value<<" isPercent = "<<percent<<endl;
		if (percent) {
			if (param == "width") {
			...
			target = stof(value);
			//melhorou bastante sem o codigo abaixo
/*			verificar ate que ponto isto ta certo (rg) obs01
			if (param == "width") {
	...
	//added by rg
	int ExecutionObject::getCurrentValue(LayoutRegion *region, string param){
		int current;
		if (param == "width") {
	...
	bool ExecutionObject::animeStep(
		...
		current = getNextStepValue(
				initValue, target,
				factor, time, initTime, (dur * 1000), stepSize);
		...
		//comment this
//		cout << "ExecutionObject::animeStep value '" << initValue << "'";
//		cout << " initTime '" << initTime << "' time '" << time << "'";
//		cout << " dur '"<< dur << "'";
//		cout << " target '" << target << "' current '" << current << "'";
//		cout << " param '" << param << "'" << endl;

		if (time >= (initTime + (dur * 1000)) ) { //if the animation time is over (rg)
			if (param == "width") {
